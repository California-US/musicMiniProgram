...
Page({
  ...
  onLoad: async function (options) {
    ...
    if (options.type == 'components') {
      ...
      let info = await request('/song/detail', {
        ids: options.song
      })
      info = info.songs[0]
      // 判断搜索歌曲是否已经在歌单中存在了 不存在就将当前音乐插入歌单，存在就不做操作
      if (!playingMusicList.some(item => {
    ...
    if (isPlay) {
      if (!link) {
        ...
        let musicLinkData = await request('/song/url', {
          id,
          br: 320000,
          cookie: wx.getStorageSync('cookie') // 关键：携带VIP认证信息
        })
        
        // 检查是否是VIP歌曲但未开通会员的情况
        if (musicLinkData.data && musicLinkData.data[0] && 
            musicLinkData.data[0].fee === 1 && 
            !musicLinkData.data[0].url) {
          wx.showToast({
            title: 'VIP歌曲需开通会员',
            icon: 'none',
            duration: 3000,
        ...
        if (!musicLinkData.data || !musicLinkData.data[0] || !musicLinkData.data[0].url) {
          wx.showToast({
            title: '当前歌曲无法播放',
            icon: 'none',
            success: () => {
  ...
  async switchShow() {
    ...
    if (isLyricsShow == true && this.data.lyrics.length <= 1) {
      // 获取歌词
      let lyrics = await request('/lyric', {
